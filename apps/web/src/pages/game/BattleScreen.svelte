<script lang="ts">
  ...
  // タッチキーボード表示中は .stage-viewport の下端を予約し、画面下部固定のキーボードと
  // ステージ(手札)の重なりを防ぐ(theme.css の --touch-kbd-reserve)。
...
<style>
  /* オーバーレイ(position:absolute; inset:0)の位置基準。transform 済みで包含ブロックには
     なるが、意図を明示するため relative を付ける。
